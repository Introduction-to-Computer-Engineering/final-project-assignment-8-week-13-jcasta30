...
#### 3A: The master can initiate the transfer over the I2C bus, but the slaves cannot. The master also drives the clock. The clock signal is always generated by the current bus master; some slave devices may force the clock low at times to delay the master sending more data (or to require more time to prepare data before the master attempts to clock it out). 

4Q. How are the two types of protocol frames different?

#### 4A: Messages are broken up into two types of frame: an address frame, where the master indicates the slave to which the message is being sent, and one or more data frames, which are 8-bit data messages passed from master to slave or vice versa. Data is placed on the SDA line after SCL goes low, and is sampled after the SCL line goes high. The time between clock edge and data read/write is defined by the devices on the bus and will vary from chip to chip.The address frame is always first in any new communication sequence.After the address frame has been sent, data can begin being transmitted.

5Q. What is the most appropriate trigger for capturing an I2C frame on the oscilloscope?

#### 5A: The most appropriate trigger for capturing an I2C frame on the oscilloscope is on the falling edge of the SDA. The SDA leads with a falling edge, and the data frame starts after the address frame and so will the SCL. During normal data writing operation, the value on SDA should not change when SCL is high, to avoid false stop conditions.
...
#### 6A: Both micro:bits will be configured by default as a master. When connected to each other via the SDA and SCL lines, having two masters will cause problems. In a 10-bit addressing system, two frames are required to transmit the slave address. As with a normal 7-bit transfer, another transfer begins immediately, and this transfer contains bits 7:0 of the address. At this point, the addressed slave should respond with an ACK bit. If it doesn't, the failure mode is the same as a 7-bit system. Both masters will not initiate/sync transmission data at the same time.
